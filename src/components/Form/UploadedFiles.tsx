...
export default function UploadedFiles({
  ...
  const [currentPreviewedItem, setCurrentPreviewedItem] =
  ...
  return (
    <>
        ...
        {files.map((file, index) => (
          <SwiperSlide
            ...
            className={clsx(
              ...
              'hover:bg-gray-100',
              'cursor-pointer',
              (!file.isImage || files.length === 1) && 'p-2',
            )}
            onClick={() => setCurrentPreviewedItem(file)}
          >
            {file.isImage && file.previewSrc ? (
              <Image
                src={file.previewSrc}
                alt={file.name}
                width={files.length > 1 ? 56 : 144}
                height={files.length > 1 ? 56 : 144}
                className={clsx(
                  'object-cover',
                  files.length > 1 ? 'h-[56px] rounded-xl' : 'h-[144px]',
                )}
              />
            ) : (
              <a
                href={!file.isImage ? file.previewSrc : undefined}
                download={file.name}
      ...
      {currentPreviewedItem?.isImage && (
        <PhotoPreview
          file={currentPreviewedItem}
          setCurrentPreviewedItem={setCurrentPreviewedItem}
        />
      )}
    </>
  )
